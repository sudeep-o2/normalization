...
def identify_base_shift(folder_path):
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_shift['percentile_score'] = df_shift['raw_score'].apply(
            lambda raw_score: calculate_percentile_score(raw_score, df_shift['raw_score'])
        )
...
def identify_base_shift(folder_path):
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_shift['percentile_score'] = df_shift.apply(
            lambda row: calculate_percentile_score(row['raw_score'], df_shift['raw_score']),
            axis=1
        )
...
def process_folder(folder_path, base_shift):
    ...
    for file_name in os.listdir(folder_path):
        ...
        # Filter rows corresponding to the Base Shift
        base_shift_rows = df_shift.sort_values(by='percentile_score')
        ...
        for index, row in df_shift.iterrows():
            ...
            base_shift_rows_filtered = base_shift_rows[
                (base_shift_rows['percentile_score'] <= x) |
                ((base_shift_rows['percentile_score'] > x) & (base_shift_rows['percentile_score'] <= x))
            ].head(2)
